...
import lms.kiu.notifier.mongo.service.BugReportService;
...
public class BotService {
  ...
  @Async
  public void sendRegistrationInstructionsAsync(BaseAbilityBot kiuNemoBot, long chatId) {
  ...
  public Mono<Void> initializeStudentAsync(MessageContext ctx) {
    ...
    silent.send("Starting Initialization", chatId);
  ...
  public Mono<Void> sendBugReport(MessageContext ctx) {
    return bugReportService
        .save(
            BugReport.builder()
                .telegramId(ctx.chatId())
                .message(ctx.update().getMessage().getText())
                .build())
        .doFirst(() -> ctx.bot().getSilent().send("Generating Bug Report...", ctx.chatId()))
        .doOnSuccess(v -> ctx.bot().getSilent().send("Bug Report sent", ctx.chatId()))
        .then();
  }
}
