...
import static lms.kiu.notifier.data.Constants.ABOUT_MESSAGE;
import static lms.kiu.notifier.data.Constants.HELP_MESSAGE;
import static lms.kiu.notifier.data.Constants.WELCOME_MESSAGE;
import static lms.kiu.notifier.utils.MessageUtils.hasMessageWith;
import static org.telegram.telegrambots.abilitybots.api.objects.Locality.ALL;
import static org.telegram.telegrambots.abilitybots.api.objects.Privacy.PUBLIC;

import lms.kiu.notifier.data.Constants;
import lms.kiu.notifier.lms.api.service.LMSService;
import lms.kiu.notifier.telegram.bot.service.BotService;
...
public class KiuNemoBot extends AbilityBot {
  ...
  public Ability report() {
    return Ability.builder()
        .name("report_bug")
        .info("Report a bug to the Admin")
        .privacy(PUBLIC)
        .locality(ALL)
        .action(ctx -> botService.sendBugReport(ctx).subscribe())
        .build();
  }
  ...
  public ReplyFlow registrationFlow() {
    return ReplyFlow.builder(db)
        .action(
            (bot, upd) ->
                botService.sendRegistrationInstructionsAsync(bot, upd.getMessage().getChatId()))
    ...
    return Reply.of(
        (bot, upd) ->
            botService
                ...
                .doOnSuccess(
                    s -> silent.send("✅ Registration successful! \n next step: /init_student", upd.getMessage().getChatId()))
                .doOnError(
  ...
  public Ability initializeStudent() {
    return Ability.builder()
        .name("init_student")
        .info(
            "extract all enrolled courses for later use in checking news")
        .locality(ALL)
        .privacy(PUBLIC)
        .action(
            ctx ->
                botService
                    ...
                    .doOnSuccess(
                        v -> silent.send(Constants.STUDENT_INITIALIZATION_SUCCESS, ctx.chatId()))
                    .doOnError(
                        error -> {
                          log.error("Error initializing student", error);
                          silent.send("An error occurred during initialization.", ctx.chatId());
                        })
                    .subscribe())
        .build();
  }
