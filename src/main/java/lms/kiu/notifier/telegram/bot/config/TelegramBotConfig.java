package lms.kiu.notifier.telegram.bot.config;

import lms.kiu.notifier.lms.api.service.LMSService;
import lms.kiu.notifier.mongo.service.BugReportService;
import lms.kiu.notifier.mongo.service.CourseService;
import lms.kiu.notifier.mongo.service.StudentService;
import lms.kiu.notifier.telegram.bot.service.BotService;
import lms.kiu.notifier.telegram.bot.service.NewsScheduler;
import org.springframework.beans.factory.annotation.Value;
import org.springframework.context.annotation.Bean;
import org.springframework.context.annotation.Configuration;
import org.springframework.security.crypto.encrypt.TextEncryptor;
import org.telegram.telegrambots.abilitybots.api.bot.AbilityBot;
import org.telegram.telegrambots.client.okhttp.OkHttpTelegramClient;
import org.telegram.telegrambots.meta.generics.TelegramClient;

@Configuration
public class TelegramBotConfig {

  @Value("${telegram.bot.username}")
  String botUsername;

  @Value("${telegram.bot.token}")
  private String botToken;

  @Bean
  public String botUsername() {
    return botUsername;
  }

  @Bean
  public TelegramClient telegramClient() {
    return new OkHttpTelegramClient(botToken);
  }

  @Bean
  public BotService botService(
      StudentService studentService,
      CourseService courseService,
      TextEncryptor textEncryptor,
      LMSService lmsService,
      BugReportService bugReportService) {
    return new BotService(
        studentService, courseService, textEncryptor, lmsService, bugReportService);
  }

  @Bean
  public NewsScheduler newsScheduler(
      AbilityBot kiuNemoBot, StudentService studentService, BotService botService) {
    return new NewsScheduler(kiuNemoBot, studentService, botService);
  }
}
