...
public class LMSService {
  ...
  public Mono<RegistrationGroupIdResponse> getRegistrationGroupIdResponse(String studentToken) {
    return webClient
        ...
        .retrieve()
        .bodyToMono(RegistrationGroupIdResponse.class);
  }
                      ...
                      courseRequest ->
                          webClient
                              ...
                              .onErrorResume(
                                  error -> {
                                    log.error(
                                        "Error fetching announcements: {}", error.getMessage());
                                    return Flux.empty();
                                  }))
                  ...
                  .flatMap(
                      courseAndRequest ->
                          webClient
                              ...
                              .flatMap(
                                  res ->
                                      Flux.fromIterable(res.getData())
                                          .filter(
                                              data ->
                                                  data.getUpdatedAt()
                                                      .isAfter(student.getLastCheck()))
                                          .map(
                                              dataItem ->
  ...
  public Mono<CoursesInfoResponse> getStudentCoursesInfo(
      String studentToken, GetCoursesInfoRequest getCoursesInfoReq) {
    return webClient
        .post()
